			...
			if (!$Format)
			{
				...
				else
				{
					...
					else
					{
						...
						$Matches = array();
						if (preg_match('{(0+)(\.?)(0*)}', (string) preg_replace('{\[[^\]]+\]}', '', $Format['Code']), $Matches))
						{
							$Integer = $Matches[1];
					...
					$Matches = array();
					if (preg_match('{\[\$(.*)\]}u', (string) $Format['Code'], $Matches))
					{
			...
			if ($Format)
			{
				...
				else
				{
					...
					if ($Format['Type'] == 'Fraction' && ($Value != (int)$Value))
					{
						...
						if (
							strpos((string) $Format['Code'], '0') !== false || 
							strpos((string) $Format['Code'], '#') !== false ||
							substr((string) $Format['Code'], 0, 3) == '? ?'
						)
						{
					...
					else
					{
						...
						if (!empty($Format['MinWidth']) && $Format['Decimals'])
						{
							...
							$Value = preg_replace('{(0+)(\.?)(0*)}', $Value, (string) $Format['Code']);
						}
					}
				...
				while ($this -> Valid = $this -> Worksheet -> read())
				{
					switch ($this -> Worksheet -> name)
					{
						...
						case 'c':
							...
							$Letter = preg_replace('{[^[:alpha:]]}S', '', (string) $Index);
