...
class ImportStructureV5 extends ImportFromFile
{
    /** @var LSActiveRecord  */
    public $currentModel;
    ...
    private function saveQuestions(){
        ...
        foreach ($this->languages as $language) {
            ...
            $this->saveQuestionAttributes($language);
        }
        $this->questionOrder ++;
     ...
     * @throws Exception
     */
    private function saveQuestionAttributes($language = null)
    {
        $column = !empty($language) ? self::COLUMN_OPTIONS . "-" . $language : self::COLUMN_OPTIONS;
        $importUnknownAttributes = true;
        if(!isset($this->rowAttributes[$column])) {
            return;
        }

        $attributeInput =$this->rowAttributes[$column];
        $attributeArray = (array) json_decode((string) $attributeInput);
        ...
        foreach ($myAttributes->attributes as $attributeName => $value) {
            ...
            $this->saveQuestionAttribute($attributeName, $value, $language);
        }

        if ($importUnknownAttributes) {
    ...
    private function saveQuestionAttribute(string $attributeName, $value, $language = null)
    {
        if (empty($language)) {
            $language = null;
        }
